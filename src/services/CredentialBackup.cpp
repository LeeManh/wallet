...
namespace {
    fs::path usersDataPath() { return fs::path(DATA_DIR) / "users.json"; }
...
void CredentialBackup::exportToFile(const std::string& outPath) {
    const fs::path src = usersDataPath();
    const fs::path dst = outPath;
    ensureParent(dst);

    if (!fs::exists(src)) {
        std::ofstream ofs(dst); ofs << "[]";
        utils::MessageHandler::logMessage("users.json không tồn tại; đã tạo file rỗng -> " + dst.string());
        return;
    }

    fs::copy_file(src, dst, fs::copy_options::overwrite_existing);
    utils::MessageHandler::logMessage("Đã sao lưu credentials -> " + dst.string());
}

void CredentialBackup::importFromFile(const std::string& inPath) {
    const fs::path src = normalizePath(inPath);
    const fs::path dst = usersDataPath();
    ...
    fs::copy_file(src, dst, fs::copy_options::overwrite_existing);
    utils::MessageHandler::logMessage("✅ Đã phục hồi credentials <- " + src.string());
}
